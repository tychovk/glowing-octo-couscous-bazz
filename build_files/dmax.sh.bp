...
BIN_DIR="/var/usrlocal/bin" #changed from /usr/local/bin
...
#  make -C /usr/lib/modules/$KVER/build M=$(pwd) -e clean # before: make clean
make -C /lib/modules/$KVER/build M=$(pwd) KVER=$KVER KDIR=$KDIR MDIR=$MDIR V=1 clean
#  make -C /usr/lib/modules/$KVER/build M=$(pwd) # before: make
make -C /lib/modules/$KVER/build M=$(pwd) KVER=$KVER KDIR=$KDIR MDIR=$MDIR V=1
make -C /lib/modules/$KVER/build M=$(pwd) KVER=$KVER KDIR=$KDIR MDIR=$MDIR V=1 install
# make install -C /usr/lib/modules/$KVER/build M=$(pwd) # before: make install
  [ -f /lib/modules/$KVER/kernel/drivers/platform/x86/linuwu_sense.ko ] && echo “linuwu file exists in /lib/modules” || echo “linuwu file does not exist”
  [ -f /usr/lib/modules/$KVER/kernel/drivers/platform/x86/linuwu_sense.ko ] && echo “linuwu file exists in /usr/lib/modules” || echo “linuwu file does not exist”
...
check_system() {
  ...
  local kernel_major=$(echo $KVER | cut -d. -f1)
  local kernel_minor=$(echo $KVER | cut -d. -f2)
